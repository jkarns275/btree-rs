...
impl<K, V> PBTree<K, V>
    where   K: RawSerialize + RawDeserialize + Eq + Ord,
            V: RawSerialize + RawDeserialize {
    ...
    pub fn open<S: Into<String>>(_path: S) -> Result<Self, Error> {
        let path = _path.into();
        let mut treefile;
        check!(OpenOptions::new().read(true).write(true).create_new(false).open(path.clone() + ".tree"), treefile);

        let keyfile;
        check!(OpenOptions::new().read(true).write(true).create_new(false).open(path.clone() + ".key"), keyfile);

        let valfile;
        check!(OpenOptions::new().read(true).write(true).create_new(false).open(path.clone() + ".val"), valfile);
